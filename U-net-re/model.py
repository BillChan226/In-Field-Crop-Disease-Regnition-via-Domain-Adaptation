...
from data_generator import gen_train_data, test_data
...
# test_data = test_data()
# model.fit_generator(train_data, steps_per_epoch=10000, epochs=16, verbose=1, 
#                     shuffle=True, validation_data=test_data)

# #%% continue to train from weight
# model = get_unet(path_weight)
# train_data = gen_train_data()
# test_data = test_data()
# model.fit_generator(train_data, steps_per_epoch=10000, epochs=4, verbose=1, 
#                     shuffle=True, validation_data=test_data)

#%% predict 
model = get_unet(path_weight)
test_data_img, test_data_img_ano = test_data()
y_pred = model.predict(test_data_img, verbose=1)
...
for img in y_pred:
    img = img[:,:,0]
    print(np.sum(img>=0.5))

# %% 叶片面积
for img in test_data_img:
    # 转化为灰度图像
    img_gray = (img[:,:,0] + img[:,:,1] +img[:,:,2]) / 3
    print(np.sum(img_gray > 1e-5))
# %% 实际病斑面积
for img in test_data_img_ano:
    img = img[:,:,0]
    print(np.sum(img>=0.5))
