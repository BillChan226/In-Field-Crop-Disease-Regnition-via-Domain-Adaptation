    ...
    def __init__(self, in_channels, out_channels, stride):
        ...
        if stride != 1 or in_channels != out_channels:
            self.residual = nn.Sequential(
                ...
                nn.BatchNorm2d(int(out_channels / 2)), nn.ReLU(inplace=True))
...
class ShuffleNetV2(nn.Module):
    def __init__(self, ratio=0.5, class_num=38):
        super().__init__()
        if ratio == 0.5:
        ...
        self.pre = nn.Sequential(nn.Conv2d(3, 24, 3, padding=1),
                                 nn.BatchNorm2d(24))
        ...
        self.conv5 = nn.Sequential(
            ...
            nn.BatchNorm2d(out_channels[3]), nn.ReLU(inplace=True))
    ...
    def forward(self, x):
        ...
        x = x.view(x.size(0), -1)
        self.features_map = x.detach() # batch_size * 1024
        x = self.fc(x)
...
if __name__ == '__main__':
    ...
    vis_graph.render('model_visual', view=False)
